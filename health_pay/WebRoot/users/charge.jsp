      ...
      <form action="RegisterServlet" method="post" name="form2">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="3">
          ...
          <tr class="tabletd<%=i%2+1 %>">
            <td align="center"><a title="点击查看该药品详情" href="users/medicine_show.jsp?mark=show&id=<%=medicine.get(0) %>"><%=medicine.get(1) %></td>
            <td align="center"><%=medicine.get(4) %>元</td>
            <td align="center"><%=alRow.get(3) %></td>
            <td align="center"><%=v.getRround(Float.parseFloat(medicine.get(4).toString())*Float.parseFloat(alRow.get(3).toString())) %>元</td>
          </tr>
          <%
          f += Float.parseFloat(medicine.get(4).toString())*Float.parseFloat(alRow.get(3).toString());
          } %>
          <tr>
            <td colspan="4" align="center">费用总计：<%=v.getRround(f) %>元</td>
		  </tr>
        </table><br>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td align="center"><a href="ChargeServlet?pathographyid=<%=pathographyid %>">提交药费单</a></td>
          </tr>
        </table>
		<%} %>
      </form></td>
